...
DATA_DIR = "./rawInput/train-tif-sample"
LABEL_FILE = "./rawInput/train.csv"

TRAIN_LOG_DIR = "./train-tif"

os.makedirs(os.path.join(TRAIN_LOG_DIR, "archive"), exist_ok=True)
os.makedirs(os.path.join(TRAIN_LOG_DIR, "tensorboard"), exist_ok=True)
...
class CNN(object):
	...
	def fit(self):
		...
		csv_logger = CSVLogger(os.path.join(TRAIN_LOG_DIR, 'training.log'))
		checkpoint = ModelCheckpoint(filepath=os.path.join(TRAIN_LOG_DIR, 'checkpoint.hdf5'), monitor='binary_crossentropy', verbose=1, save_best_only=True)
		tensorboard = TensorBoard(log_dir=os.path.join(TRAIN_LOG_DIR, 'tensorboard'), histogram_freq=1, write_graph=True, write_images=True, embeddings_freq=1)
...
class Dataset(object):
	...
	def __init__(self, list_files, labels_file, train_idx=None):
		...
		if train_idx is None:
			...
			with open(os.path.join(TRAIN_LOG_DIR, 'train-idx.csv'), 'w') as f:
		...
		copyfile(os.path.join(TRAIN_LOG_DIR, 'train-idx.csv'), os.path.join(TRAIN_LOG_DIR, 'archive/{f}-train-idx.csv'.format(f=sessionId)))
...
if __name__ == "__main__":
	with K.get_session():
		...
		list_imgs = [os.path.join(DATA_DIR, f) for f in list_imgs if (f.endswith(".jpg") or f.endswith(".tif"))]

		if args["model"] != '':
			with open(os.path.join(TRAIN_LOG_DIR, 'train-idx.csv')) as f_train_idx:
		...
		copyfile(TRAINED_MODEL, os.path.join(TRAIN_LOG_DIR, "archive/{f}-model.h5".format(f=sessionId)))
		print('Done running')
